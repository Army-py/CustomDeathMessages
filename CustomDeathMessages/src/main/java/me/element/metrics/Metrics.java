...
public class Metrics {
        ...
        for (Class<?> service : (Iterable<Class<?>>) Bukkit.getServicesManager().getKnownServices()) {
            try {
                ...
                for (RegisteredServiceProvider<?> provider : Bukkit.getServicesManager().getRegistrations(service)) {
                    try {
                        Object plugin = provider.getService().getMethod("getPluginData", new Class[0])
                                .invoke(provider.getProvider(), new Object[0]);
    ...
    public static void sendData(Plugin plugin, JsonObject data) throws Exception {
        ...
        HttpsURLConnection connection = (HttpsURLConnection) (new URL("https://bStats.org/submitData/bukkit"))
                .openConnection();
        ...
        private JsonObject getRequestJsonObject() {
            ...
            try {
                ...
                if (Metrics.logFailedRequests)
                    Bukkit.getLogger().log(Level.WARNING, "Failed to get data for custom chart with id " + this.chartId,
                            t);
                return null;
            }
            return chart;
